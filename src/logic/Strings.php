...
`29,`1300.00,`490.00,`0.00,`4.33000,`1300.00,`490.00
    ...
    public static function dataReplace($str, $data, $keyPreFix = '') {
        ...
        foreach ($data as $key => &$value) {
            ...
            if(is_array($value) || is_object($value)){
                // continue;
                // 递归
                $str = static::dataReplace($str, $value, $key.'.');
            } else {
                // 不是数组的才替换
                // 20250207:处理null
                $value = $value ? : '';
                $str = $str ? str_replace('{$' . $keyPreFix.$key . '}', $value, $str) : $str;
            }
        }
        return $str;
    }
     ...
     * @return type
     */
    public static function uncamelize($input, $separator = '_') {
        // return strtolower(preg_replace('/([a-z])([A-Z])/', "$1" . $separator . "$2", $camelCaps));
        return strtolower(preg_replace_callback('/(?<!^)[A-Z]/', function ($match) use ($separator) {

...
class DataCheck {
    ...
    public static function must($data, $keys, $notices = []) {
        foreach ($keys as $key) {
            ...
            if (!isset($data[$key]) || !$data[$key]) {
                throw new Exception(isset($notices[$key]) ? $notices[$key] : $key . '必须');
            }
        }
    }
    ...
    public static function effect($data, $effects, $notices = []) {
        foreach ($effects as $key => $effVals) {
            ...
            if (!in_array(Arrays::value($data, $key), $effVals)) {
                throw new Exception(isset($notices[$key]) ? $notices[$key] : $key . '不是有效的值');
            }
        }
    }
    ...
    public static function isNumber($data, $keys, $notices = []) {
        foreach ($keys as $key) {
            ...
            if (isset($data[$key]) && !is_numeric($data[$key])) {
                throw new Exception(isset($notices[$key]) ? $notices[$key] : $key . '不是一个数字');
            }
        }
    }
    ...
    public static function maxCharLength($dataStr, $mLength, $errMsg) {
        if (is_array($dataStr)) {
            throw new Exception('$dataStr必须是字符串');
        }
        if ($dataStr && $mLength && mb_strlen($dataStr) > $mLength) {

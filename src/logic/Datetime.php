...
class Datetime {
    ...
    public static function monthlyDates($month) {
        ...
        return static::getWithinDate($startTime, $endTime);
    }
    ...
    public static function monthlyScopeTimes($month) {
        $startTime = static::monthStartTime($month);
        $endTime = static::monthEndTime($month);
    ...
    public static function dateScopeTimes($date) {
        $startTime = static::dateStartTime($date);
        $endTime = static::dateEndTime($date);
        return [$startTime, $endTime];
    }
    ...
    public static function yearMonthTimeCon($key, $yearmonth, $day = '') {
        ...
        if ($day) {
            //①日
            $day = $yearmonth ? $yearmonth . '-' . $day : date('Y-m') . '-' . $day;
    ...
    public static function scopeTimeStr($startTime, $endTime) {
        ...
        if ($startMonth == $endMonth && strtotime($startTime) == strtotime(static::monthStartTime($startMonth)) && strtotime($endTime) == strtotime(static::monthEndTime($endMonth))) {
            return date('Y年m月', strtotime($startTime));
        }
        // 如果是当年的1月1号至当年的12月31号，返回年份
        if ($startYear == $endYear && strtotime($startTime) == strtotime(static::yearStartTime($startYear)) && strtotime($endTime) == strtotime(static::yearEndTime($endYear))) {
            return $startYear . '年';
        }
    ...
    public static function calAge($birthday, $time) {
        return static::calTimeState($birthday, $time, 1);
    }
    ...
    public static function paramScopeTime($param) {
        if (Arrays::value($param, 'yearmonthDate')) {
            return static::dateScopeTimes($param['yearmonthDate']);
        }
        if (Arrays::value($param, 'yearmonth')) {
            ...
            if (!$date) {
                return static::monthlyScopeTimes($param['yearmonth']);
            } else {
                return static::dateScopeTimes($yearmonth . '-' . $date);
            }
        }
        if (Arrays::value($param, 'year')) {
            return static::yearlyScopeTimes($param['year']);
        }
        // 20240404
        if (Arrays::value($param, 'yearweek')) {
            return static::weeklyScopeTimes($param['yearweek']);
        }
    ...
    public static function paramTimeDataForMerge($param, $timeField = '') {
        ...
        //日期字符串，用于展示
        $data['DTDateStr'] = $scopeTimeArr ? static::scopeTimeStr($scopeTimeArr[0], $scopeTimeArr[1]) : '全部';
     ...
     * @param type $microRand
     */
    public static function timestamp($datetimeStr, $microRand = false){
        $time       = strtotime($datetimeStr);
        $timeStr    = date('Y-m-d',$time);
        // 20250914:当天的随机时间都可以，减少碰撞概率
        $timeRand = strtotime($timeStr) + rand(0,86399);
        if($microRand){
            $timeRand += rand(0,999) * 0.001;
        }
        return $timeRand;
    }

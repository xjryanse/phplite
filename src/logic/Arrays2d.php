...
class Arrays2d
{
    ...
    */
    ...
    public static function listCount($listsAll, $con = [] ){
        return count(static::listFilter($listsAll, $con));
    }
    /**
     * 列表数据过滤,取单条
     * @param type $listsAll 二维数组数据
    ...
    public static function dataListFind($listsAll, $data){
        ...
        return static::listFind($listsAll, $con);
    }
    ...
    public static function maxFind( $listsAll, $judgeField, $con = [] ){
        $arrays = static::listFilter($listsAll, $con);
        $arr = static::sort($arrays, $judgeField, 'desc');
        return $arr ? $arr[0] : [];
    }
    /**
     * 查某列的最小值，取整行数据
    ...
    public static function minFind( $listsAll, $judgeField, $con = [] ){
        $arrays = static::listFilter($listsAll, $con);
        $arr = static::sort($arrays, $judgeField, 'asc');
        return $arr ? $arr[0] : [];
    }
    ...
    public static function sort( &$array ,$field, $sort="asc"){
        ...
        $sortFlag = strtolower($sort) === 'desc' ? -1 : 1;
        usort($array, function($a, $b) use ($field, $sortFlag) {
            // 检查字段是否存在
            if (!isset($a[$field]) || !isset($b[$field])) {
                throw new \InvalidArgumentException("排序字段 '$field' 不存在于数组元素中");
            }
            ...
            // 字符串比较
            $compareResult = strcmp($strA, $strB);

            return $compareResult * $sortFlag;
        });
...
//            $picArr = $picObjs ? $picObjs->toArray() : [];
    ...
    public static function mixPicFieldCov($listsAll, $mixPicFields ){
        foreach($mixPicFields as $pFields){
            ...
            $arr        = static::listFilter($listsAll, $con);
            $covData    = static::picFieldCov($arr, $pFields['cKey']);
            $covDataObj = static::fieldSetKey($covData, 'id');
            // 赋值
    ...
    public static function groupCount($lists ,$group){
        $keys = static::uniqueColumn($lists, $group);
        $cArr = [];
        foreach($keys as $key){
            $con        = [];
            $con[]      = [$group,'=',$key];
            $cArr[$key] = count(static::listFilter($lists, $con));
        }
        return $cArr;
    }
    /**
    ...
    public static function groupSum($lists, $group, $sumField){
        ...
        foreach($keys as $key){
            $con        = [];
            $con[]      = [$group,'=',$key];
            $cArr[$key] = static::sum(static::listFilter($lists, $con),$sumField);
        }
        return $cArr;        
    }
    ...
    public static function makeTree($arr,$pid='',$pidname='pid',$child='list')
    {
        ...
        foreach ($arr as $item) {
            $iName = $item[$pidname] ? : '';
            if($iName == $pid){
                $item[$child] = static::makeTree($arr,$item['id'], $pidname, $child);
                $trees[] = $item;
            }
        }
    ...
    // 使用动态列表替换
    public static function dynDataReplace($arr,$dynDataList){
        foreach($arr as &$v){
            foreach($v as $k1=>&$v1){
                // 获取键值对
                $dynValObj = Arrays::value($dynDataList, $k1) ? : [];
                // 替换
                $v[$k1] = $dynValObj && isset($dynValObj[$v1]) ? $dynValObj[$v1] : $v1;
            }
        }
        return $arr;
    }
    ...
    /**
     * 20250425:计算数组的偏差
     * @param type $interfaceData   接口新数据
     * @param type $localData       本系统数据
     */
    public static function calDiffs($interfaceData, $localData, $uField = 'id', $keyField = 'id'){
        $iData          = array_values($interfaceData);
        $tData          = array_values($localData);
        
        $intersectedKeys = count($iData) && count($tData) 
                ? array_keys(array_intersect_key($iData[0], $tData[0])) 
                : [];

        $iDataIndexed   = Arrays2d::fieldSetKey($iData, $uField);
        $tDataIndexed       = Arrays2d::fieldSetKey($tData, $uField);

        // 找出需要更新的数据
        $toAdd      = [];
        $toUpdate   = [];
        foreach ($iDataIndexed as $id => $interfaceItem) {
            // 【1】本地没有，待添加
            if (!isset($tDataIndexed[$id])) {
                $toAdd[] = $interfaceItem;
                continue;
            }
            $localItem = $tDataIndexed[$id];
            // 【2】本地有，且不一致待更新
            $dI     = Arrays::getByKeys($interfaceItem, $intersectedKeys);
            $dL     = Arrays::getByKeys($localItem, $intersectedKeys);
            $res    = Arrays::diffArr($dL, $dI);
            if ($res) {
                $item               = $interfaceItem;
                // 把id拼接上去
                $item[$keyField]    = $localItem[$keyField];
                $toUpdate[]         = $item;
            }
        }
        // 找出需要删除的数据
        $toDelete = [];
        foreach ($tDataIndexed as $id => $localItem) {
            if (!isset($iDataIndexed[$id])) {
                $toDelete[] = $localItem;
            }
        }

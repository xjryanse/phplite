...
class ModelQueryCon {
    ...
    public static function conditionParse($con, $alias = "", $keyReplace = []) {
        ...
        $condition = [];
        foreach ($con as $v) {
            $condition[] = static::conditionParseItem($v, $alias, $keyReplace);
        }
    ...
    public static function conditionParseItem($conItem, $alias = "", $keyReplace = []) {
        // 20250203：增加有·判断，兼容别名
        $conItem[0] = Strings::hasStr($conItem[0], '.') ? $conItem[0] : '`' . $conItem[0] . '`';
        if ($conItem[1] == "in") {
            if (is_string($conItem[2]) || is_numeric($conItem[2])) {
                $conItem[2] = [$conItem[2]];
            }
            $conItem[2] = "('" . implode('\',\'', $conItem[2]) . "')";
        } else if ($conItem[1] == "is"){
            // 20250225：is不加引号
            $conItem[2] = $conItem[2];
        } else {
            $conItem[2] = '\'' . $conItem[2] . '\'';
    ...
    public static function queryCon(array $param, array $fields, $style = "where") {
        ...
        foreach ($fields as $k => &$v) {
            ...
            foreach ($v as $key => &$value) {
                ...
                //解析成where;
                $tmp = static::condition($k, $key, $param, $value);
                if ($tmp) {
        ...
        switch ($k) {
            case "equal": //精确查找
                if (isset($param[$value]) && !is_array($param[$value]) && strlen($param[$value])) {
                    $con[] = [$key, '=', static::preg($param[$value])];
                }
                break;
            case "like": //模糊查找
                if (isset($param[$value]) && strlen($param[$value])) {
                    $con[] = [$key, 'like', '%' . static::preg($param[$value]) . '%'];
                }
                break;
            case "in": //in查找
